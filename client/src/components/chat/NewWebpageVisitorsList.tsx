...
const NewWebpageVisitorsList = ({
  ...
  const getStatusColor = useCallback((status: UserStatus): string => {
    switch (status) {
  ...
  const getDomainFromUrl = useCallback((urlString: string): string => {
    try {
      const urlObject = new URL(urlString);
      return urlObject.hostname;
    } catch {
      // Ritorna una parte dell'URL o un valore default se l'URL non è valido
      return urlString.replace(/^https?:\/\//, '').split('/')[0] || 'sito web';
    }
  }, []);

  const domain = useMemo(() => getDomainFromUrl(url), [url, getDomainFromUrl]);

  const handleStartPrivateChat = useCallback((nickname: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent triggering the expand/collapse

    if (nickname !== currentUser) {
      onStartPrivateChat(nickname);
    }
  }, [currentUser, onStartPrivateChat]);

  const toggleExpandedVisitor = useCallback((socketId: string) => {
    setExpandedVisitor(prevId => prevId === socketId ? null : socketId);
  }, []);

  const hasUnreadMessages = useCallback((visitor: WebpageVisitor): boolean => {
    return Boolean(
  ...
  return (
      <div className="bg-white rounded-lg shadow-md overflow-hidden h-full flex flex-col">
        <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center text-sm text-gray-600 hover:text-gray-900 focus:outline-none" aria-label="Set your status">
            ...
            <DropdownMenuContent>
              ...
              <DropdownMenuItem onClick={() => onSetStatus(UserStatus.IDLE)} className="flex items-center">
                <div className="w-2 h-2 rounded-full bg-yellow-500 mr-2" aria-hidden="true"></div>
                Idle
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => onSetStatus(UserStatus.AWAY)} className="flex items-center">
                <div className="w-2 h-2 rounded-full bg-gray-400 mr-2" aria-hidden="true"></div>
                Away
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        <div className="overflow-y-auto flex-1 p-2">
          <ul className="divide-y divide-gray-100">
            {visitors.map((visitor) => (
                <li
                    key={`visitor-${visitor.nickname}-${visitor.socketId}`}
                    id={`visitor-${visitor.nickname}`}
                    className={`p-2 hover:bg-gray-50 rounded-md transition-colors ${
                        visitor.nickname === currentUser ? "bg-blue-50" : ""
                    }`}
                    onClick={() => toggleExpandedVisitor(visitor.socketId)}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className="relative">
                        ...
                        <div
                            className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-white ${getStatusColor(visitor.status)}`}
                            aria-label={`Status: ${visitor.status}`}
                        ></div>
                      </div>
                      <div className="ml-3">
                        <p className="text-sm font-medium text-gray-700 flex items-center">
                          ...
                          {visitor.nickname !== currentUser && hasUnreadMessages(visitor) && (
                          ...
                          <>
                            ...
                            {chatHistoryUsers.includes(visitor.nickname) ? (
                                <button
                                    ...
                                    aria-label={`Chat con ${visitor.nickname}`}
                                >
                                  {hasUnreadMessages(visitor) ? (
                                      <ChatBubble className="h-5 w-5" aria-hidden="true" />
                                  ) : (
                                ...
                                <button
                                    ...
                                    title="Inizia chat"
                                    onClick={(e) => handleStartPrivateChat(visitor.nickname, e)}
                                    aria-label={`Inizia chat con ${visitor.nickname}`}
                                >
